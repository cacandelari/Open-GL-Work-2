...
Vector3d Scene::trace(const Ray &ray) {
    ...
    Vector3d result;
    result[0] = result[1] = result[2] = 0;//the background color, 0 is black, 1 is white

    if (r.t < numeric_limits<double>::epsilon()) return result;

    Vector3d lightDir = (lightPos - r.position).normalized();
	/*
	To incorporate shadows, you bounce a ray off of the surface of the object
	if at any point BEFORE getting to the light source, the ray hits another object, 
	then the point at which the ray originated from becomes black, to indicated 
	the presence of a shadow and the light source being blocked
	*/
...
void Scene::render(Image &image) {
    ...
    for (int x = 0; x < size; x++)
        for (int y = 0; y < size; y++) {
            Ray curRay;
            curRay.origin[0] = curRay.origin[1] = curRay.origin[2] = 0;//changing this to 1 moved the camera
